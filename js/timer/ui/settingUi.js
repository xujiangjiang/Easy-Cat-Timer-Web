...
class SettingUi {
    ...
    OnMouseMoveVolumeSliderRound(e) {
        ...
        if (this.isDragVolumeSliderRound == true) {
            //如果鼠标的位置是0，那么就不继续运行啦
            if (e.clientX < this.uiElement.parentElement.offsetLeft)
                return;
            //获取鼠标的位置（鼠标相对于浏览器左上角的位置）
            let _mouseX = e.clientX; //X坐标
            //获取[音量滑动条的面板]的位置（面板的左上角 相对于 浏览器左上角 的位置）
            let _volumeSliderPanelX = this.volumeSliderPanelElement.offsetLeft + this.uiElement.parentElement.offsetLeft;
